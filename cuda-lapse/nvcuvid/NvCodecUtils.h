...
inline bool check(CUresult e, int iLine, const char *szFile) {
    if (e != CUDA_SUCCESS) {
		...
		logger->error("CUDA driver API error {} at line {} in file {}", szErrName, iLine, szFile);
        return false;
    }
    return true;
}
...
inline bool check(cudaError_t e, int iLine, const char *szFile) {
    if (e != cudaSuccess) {
		logger->error("CUDA runtime API error {} at line {} in file {}", cudaGetErrorName(e), iLine, szFile);
        return false;
    }
    return true;
}
...
inline bool check(NVENCSTATUS e, int iLine, const char *szFile) {
    ...
    if (e != NV_ENC_SUCCESS) {
		logger->error("NVENC error {} at line {} in file {}", aszErrName[e], iLine, szFile);
        return false;
    }
    return true;
}
...
inline bool check(HRESULT e, int iLine, const char *szFile) {
    if (e != S_OK) {
		logger->error("HRESULT error 0x{} at line {} in file {}", (void*)e, iLine, szFile);
    	return false;
    }
    return true;
}
...
inline bool check(int e, int iLine, const char *szFile) {
    if (e < 0) {
		logger->error("General error {} at line {} in file {}", e, iLine, szFile);
        return false;
    }
    return true;
}
    ...
    BufferedFileReader(const char *szFileName, bool bPartial = false) {
        ...
        while (nSize) {
            try {
                ...
                if (nSize != st.st_size) {
					logger->warn("File is too large - only {} %is loaded", 100.0 * nSize / st.st_size);                }
                break;
            } catch(std::bad_alloc) {
                if (!bPartial) {
					logger->error("Failed to allocate memory in BufferedReader");
                    return;
                }
        ...
        if (!fpIn)
        {
			logger->error("Unable to open input file: {}", szFileName);
            return;
        }
...
private:
    uint8_t *pBuf = NULL;
    uint32_t nSize = 0;
	shared_ptr<spdlog::logger> logger = create_logger("BufferedFileReader");
};
